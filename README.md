# Nearmap Roof and Solar Panel Detection

A multi-label semantic segmentation solution for detecting roofs and solar panels in aerial imagery using PyTorch.
...
├── outputs_tinyunet_bce/   # Experiment 1 results
│   ├── logs/
│   └── visualisations/
└── outputs_resnetunet_bce_dice/  # Experiment 2 results
    ├── logs/
    └── visualisations/
```
...
### Training Curves and Prediction Samples

Training loss curves, validation metrics, and example predictions with overlays are saved in the `outputs_*/visualisations/` directories for each experiment.
